...
const Table = ({ user }) => {
  ...
  const fetchBlocks = async () => {
    try {
      const res = await axios.get("http://localhost:5000/table-blocks");
      setBlocks(res.data);
  ...
  const handleAddBlock = async () => {
    ...
    try {
      await axios.post("http://localhost:5000/table-blocks", formData, {
  ...
  const handleAddDetail = async (blockId) => {
    ...
    try {
      await axios.put(`http://localhost:5000/table-blocks/${blockId}/details`, formData, {
        headers: { "Content-Type": "multipart/form-data", Authorization: `Bearer ${user.token}` },
      });
              ...
              <div className="admin-actions">
                ...
                <button onClick={async () => {
                  if(window.confirm("Видалити цей блок?")){
                    await axios.delete(`http://localhost:5000/table-blocks/${block.id}`, {
